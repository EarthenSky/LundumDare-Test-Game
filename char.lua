...
function Char.draw()
    ...
    love.graphics.print(debug_str, 200, 100)
end
...
function Char.update(dt)
    ...
    elseif love.keyboard.isDown( "a" ) then
        ...
        if food_is_picked_up == true then
            ...
            for k, val in pairs(Char.grabbed_objects) do
                if val == true then
                    foodManager.foodList[k].m_food.body:setX(foodManager.foodList[k].m_food.body:getX() - SPEED * dt)
                end
            end
        end
    ...
    if love.keyboard.isDown( "space" ) then
        if space_key == false then
            ...
            if food_is_picked_up == false then
                -- Loop food.  Drop these food.
                for k, val in pairs(foodManager.foodList) do
                    if Char.grabbed_objects[k] == true then
                ...
                for k, val in pairs(foodManager.foodList) do
                    if distance( val.m_food.body:getX(), val.m_food.body:getY(), objects.player.body:getX(), objects.player.body:getY() ) < 48 then
    ...
    if objects.player.body:getY() > 512 and objects.player.body:getY() < 640 - 32 then
        ...
        if food_is_picked_up == true then
            -- Loop food.  Drop these food.
            for k, val in pairs(Char.grabbed_objects) do
                if val == true and foodManager.foodList[k].on_belt == false then
                    foodManager.foodList[k].m_food.body:setX(foodManager.foodList[k].m_food.body:getX() + 50 * dt)
                end
            end
        end
    ...
    elseif objects.player.body:getX() > 16 and objects.player.body:getX() < 128 - 16 and objects.player.body:getY() < 128 + 16 then
        ...
        if food_is_picked_up == true then
            ...
            for k, val in pairs(Char.grabbed_objects) do
                if val == true and foodManager.foodList[k].on_belt == false then
                    foodManager.foodList[k].m_food.body:setY(foodManager.foodList[k].m_food.body:getY() - 50 * dt)
                end
            end
        end
    ...
    -- Loop grabbed food.  Check if grabbed food is off the
    hold_count = 0  -- Amount of food objects that are being held.
    for k, val in pairs(Char.grabbed_objects) do
        if val == true then
            hold_count = hold_count + 1
            if distance( foodManager.foodList[k].m_food.body:getX(), foodManager.foodList[k].m_food.body:getY(), objects.player.body:getX(), objects.player.body:getY() ) > 48 then
                Char.grabbed_objects[k] = false
            end
        end
    end

    -- If food is dropped can be picked up again without switching.
    if food_is_picked_up == true and hold_count == 0 then
        food_is_picked_up = false
    end
